...
description: |
  ...
  与 aigc-humanizer-zh MCP Server 共享同一套 16 种模式编号和 7 项硬约束体系，
  额外内置 LaTeX 公式保护/还原、TTR 词汇丰富度自检和逐段结构化输出模板。
...
**除非用户明确说"直接改写"，否则按以下流程执行：**

**Step 0：LaTeX 公式保护（如有公式）**

若文本包含 LaTeX 公式（`$...$`、`$$...$$`、`\begin{...}\end{...}`），先将每个公式块替换为语义占位符 `[MATH_1]`、`[MATH_2]`……改写完成后还原。详见下方「LaTeX 公式处理」章节。

**Step 1：逐段扫描，输出结构化风险清单**
...
│ 硬约束命中: HC-X（如有）
│ 是否需改写: 是 / 否
│ 若需改写，优先执行: 移位→砍尾→破对称→换词→去模糊→注视角
└────────────────────────────────────
```

风险等级判定：🔴 ≥4 个模式命中 ｜ 🟡 1-3 个 ｜ 🟢 0 个

**Step 2：等用户确认后逐段交互式改写**

用户选择需改写的段落后，逐段执行：
展示原文 → 输出改写版（保留核心意思 + 注入学者视角）→ 询问「是否替换？」

若用户说「直接改」或文本较短（≤300字），跳过报告直接改写。

**Step 3：改写后全文自检**

- 逐项核查 7 项硬约束（HC-1 ~ HC-7）
- 6 维度 60 分制质量自评（≥54 优秀）
- 检查噪声预算（每千字保留 2-3 处轻微 AI 特征）
- 如有公式，还原 LaTeX 占位符

若使用了 aigc-humanizer-zh MCP Server，Step 1 可调用 `analyze_by_paragraph` 自动完成，Step 3 可调用 `assess_quality` 自动评分，LaTeX 保护/还原可调用 `mask_latex` / `restore_latex`。

---

## 🔧 LaTeX 公式处理

若文本包含 LaTeX 公式（行内 `$...$`、区块 `$$...$$`、环境 `\begin{...}\end{...}`），必须按以下流程操作，防止改写时篡改公式：
...
- 行内公式 `$E=mc^2$` → `[MATH_1]`
- 区块公式 `$$\sum_{i=1}^n x_i$$` → `[MATH_2]`
- LaTeX 环境 `\begin{equation}...\end{equation}` → `[MATH_3]`

占位符使用递增编号，记录「编号 → 原始公式」的映射表。

### Step B：在骨架文本上改写

所有逐段分析、改写操作在骨架文本（含占位符）上进行。占位符保持原样不动——它们对改写逻辑透明。

### Step C：公式还原（改写后）

将骨架文本中的所有 `[MATH_N]` 按映射表还原为原始公式，输出最终文本。

> ⚠ 如果改写了包含公式的段落，务必在还原后检查公式完整性。

---

## ⚠ 强制硬约束（HC-1 ~ HC-7，命中即修复）

以下约束在输出前必须逐项核查，**命中即需修复，不可跳过**。编号与 aigc-humanizer-zh MCP Server 的 7 项硬约束一一对应。

| 编号 | 约束项 | 硬上限 | 说明 |
|------|--------|--------|------|
| HC-1 | AI高频词（模式11词表） | 每段 ≤2个 | 超出必须替换 |
| HC-2 | 段末总结套句（模式2/7触发词） | 全文 ≤1处 | 超出必须删除或改写 |
| HC-3 | 整齐三元并列 | 每段 ≤1处 | 超出必须打破对称 |
| HC-4 | 全文「依据/基于XX理论」开头段落 | ≤20%的段落数 | 超出必须移位 |
| HC-5 | 正文加粗 | 全文 ≤5处 | 超出必须削减 |
| HC-6 | 泛化结尾（模式10触发词） | 全文 0处 | 命中即修复 |
| HC-7 | 模糊归因（模式8触发词，无具体出处） | 全文 0处 | 命中即删除或具体化 |
...
改写完成后，检查全文用词是否过于单调重复（AI 写作的典型特征之一）。

**TTR（Type-Token Ratio）= 唯一词数 ÷ 总词数**

- **≥ 0.45**：词汇丰富度合格，用词自然有变化
- **0.35 ~ 0.44**：偏单调，建议在次要段落中替换 2-3 个重复高频词
- **< 0.35**：严重单调，排查是否有机械重复的术语或短语，优先处理模式 11（AI 高频词）

**快速判断法**（无需精确计数）：扫读全文，如果发现下列词在不到 500 字的文本内出现 3 次以上，即为单调信号——「进行」「通过」「影响」「表明」「体现」「作用」「显著」。
...
## 快速自检清单（最终输出前逐项核查，对齐 HC-1 ~ HC-7）

| 对应 HC | 检查项 | 标准 | 通过？ |
|---------|--------|------|------|
| HC-7 | 模糊归因 | 全文0处「专家认为」类无出处归因 | ☐ |
| HC-2 | 段末套句 | 全文≤1处「此案例XX了/由此可见」 | ☐ |
| HC-6 | 泛化结尾 | 结尾段无「具有重要意义」「前景广阔」类句子 | ☐ |
| HC-4 | 理论起笔 | ≤20%段落以「依据XX理论」开头 | ☐ |
| HC-1 | AI高频词 | 每段≤2个，优先处理词表前4项 | ☐ |
| HC-5 | 加粗 | 正文全文≤5处（不含摘要、标题） | ☐ |
